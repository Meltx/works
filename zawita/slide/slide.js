...
function t3(event) {
    if(event.target.classList[0] == 'div-3_1'){
        document.querySelector('.out-3').innerHTML = 1
    }else if(event.target.classList[0] == 'div-3_2'){
...
function t8(event) {
    ...
    event.target.style.backgroundColor = a8[random]
}
...
function t10(event) {
    c1++
    event.target.style.width = c1 + 'px'
}
...
function t11(event) {
    document.querySelector('.out-11').innerHTML = event.targetTouches[0].screenX+ ' '+ event.targetTouches[0].screenY
}
...
function prevFunction() {
  if (count == 0) {
    ...
    bigImg.src = images[images.length - 1].src;
